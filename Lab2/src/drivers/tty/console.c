...
/*
	send_updatemsg - inform TTY to update banner
*/
void
send_updatemsg(void) {
	if (jiffy % (HZ / 10) == 0) {
		...
		send(MSG_HWINTR, TTY, &m);
	}
}

/*
	init_console - set banner '    0 '
				   set current console as tty0
				   init vedio buf
				   registe irq0 handle function send_updatemsg
*/
