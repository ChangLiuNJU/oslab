...
static uint16_t *vmem = (void*)(0xb8000);
...
read_request(DevMessage *m) {
	...
	if (c->f == c->r) {
		...
		if (c->rtop >= RSTK_SZ) abort("too many read request", -1);
	} else {
		int nread = get_cooked(c, m->buf, m->count);
		m->type = nread;
		send(m->src, (Message*)m);
	}
}
...
cook(Console *c) {
	...
	do {
		c->cbuf[c->r] = *p;
		c->r = (c->r + 1) % CBUF_SZ;
		if (c->r == c->f) abort("cooked buffer full", -1);
	} while (*p ++ != 0);
	c->lbuf[c->i = 0] = 0;
	cr(c); lf(c);
	if (c->rtop != 0) {
...
consl_accept(Console *c, char ch) {
	...
	for (i = cc + 1; i > 0; i --) {
		if (c->i + i >= LBUF_SZ) abort("line buffer full", -1);
	...
	putc(c, ch);
	next(c);
	if (c->i >= LBUF_SZ) abort("line buffer full", -1);
	c->lbuf[c->i ++] = ch;
	consl_sync(c);
}
